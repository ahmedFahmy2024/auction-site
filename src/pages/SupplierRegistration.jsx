...
function SupplierRegistration() {
  ...
  const { id } = useParams();
  const { supplierRegistrations } = useUser();

  if (!supplierRegistrations) return null;
  ...
  return (
    ...
    >
      <div className="complains-container">
        {supplierRegistrations.map((complain, index) => {
          return (
            <NavLink
              to={`/settings/${id}/supplierRegistration/${complain?.id}/edit`}
              key={index}
            >
              <div className="first-row">
                ...
                <div className="thirdSec">
                  ...
                  <div className="stack">
                  <div className="my-label">{t("رقم السجل التجاري : ")}</div>
                  <div style={{ color: "#000" }} className="title">
                    {complain?.commercial_registration_number}
                  </div>
                  </div>
        ...
        <div style={{ marginTop: "20px" }}>
          <NavLink to={`/settings/${id}/supplierRegistration/add`}>
